...
// {
    if ($_POST['action'] == "add")
    {
        ...
        $query = "INSERT INTO " . PRJ_DB_TABLE_VEHICLE . " 
                (Id, Name, Maker, Model, Plate, PurchaseDate, VehicleTypeId, Active, Note) 
                VALUES (NULL, 
                '" . addslashes(stripslashes($_POST['name'])) . "',
                '" . addslashes(stripslashes($_POST['maker'])) . "', 
                '" . addslashes(stripslashes($_POST['model'])) . "', 
                '" . addslashes(stripslashes($_POST['plate'])) . "', 
                STR_TO_DATE('" . addslashes(stripslashes($_POST['purchase'])) . "','%d/%m/%Y'), 
        ...
        {
            $data['status'] = 1;
            $data['error'] = MyError::getErrorMessage(MyError::_Vehicle_ErrorAddingNew);
        }
    }
    else if ($_POST['action'] == "edit")
    {
        ...
        // FIXME: timestamp is usefull?
        // FIXME: Log, is usefull?
        $query = "UPDATE " . PRJ_DB_TABLE_VEHICLE . " SET
                 Name='" . addslashes(stripslashes($_POST['name'])) . "', 
                 Maker='" . addslashes(stripslashes($_POST['maker'])) . "', 
                 Model='" . addslashes(stripslashes($_POST['model'])) . "', 
                 Plate='" . addslashes(stripslashes($_POST['plate'])) . "', 
                 PurchaseDate=STR_TO_DATE('" . addslashes(stripslashes($_POST['purchase'])) . "','%d/%m/%Y'), 
                 VehicleTypeId = '" . addslashes(stripslashes($_POST['type'])) . "', 
        ...
        {
            $data['status'] = 1;
            $data['error'] = MyError::getErrorMessage(MyError::_Vehicle_ErrorEdit);
        }
    }
// }
// else
